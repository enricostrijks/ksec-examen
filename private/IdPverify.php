...
class IdPVerify extends JWT
{
    ...
    public function checkApiKey()
    {
        ...
        $stmt = $mysqli->prepare("SELECT * FROM users WHERE username = ? AND password = ? AND apiKey = ? ");
        $stmt->bind_param("sss", $username, $password, $apiKey);

        $username = $this->bearerCredentials['username'];
        $password = $this->bearerCredentials['password'];
        $apiKey = $this->bearerCredentials['apiKey'];
